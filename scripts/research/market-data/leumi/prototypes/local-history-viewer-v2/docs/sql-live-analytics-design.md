...
Phase F selects a snapshot-centric schema with full raw JSON, a small promoted typed set, wide core-horizon link/metric columns and a latest_snapshot pointer table. Phase G now selects one immutable validated-cycle handoff, one bulk cycle operation, set-based SQL enrichment and one atomic transaction that advances current_universe/latest_snapshot only at commit. Scheduler policy, persistence lifecycle and Viewer transport remain later planning work.
