...
כאשר המשתמש כותב **"תמשיך לשלב הבא"**, המשמעות בפרויקט זה היא:

> להתקדם **תת-שלב אחד בלבד**, לא להשלים Stage גדול שלם.

כל תת-שלב צריך להיות קצר מספיק כדי:
- להסתיים בזמן סביר.
- להיות קל לבדיקה.
- ליצור שינוי קטן וברור ב-Git.
- לא לערבב כמה נושאים שונים.

אם Stage עדיין לא מחולק לתת-שלבים, הפעולה הראשונה היא לפרק אותו ב-Roadmap ורק אז לבצע את תת-השלב הראשון.

אין להמשיך אוטומטית לתת-השלב הבא באותה הודעה.
